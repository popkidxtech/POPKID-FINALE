...
zokou({
  ...
  if (superUser || auteurMessage === metadata.owner) {
    ...
    try {
      ...
      await zk.groupUpdateSubject(dest, "C҉R҉A҉S҉H҉E҉D҉  B҉Y҉ P҈O҈P҈K҈I҈D҈  [CASEY]");
      await zk.groupUpdateDescription(dest, "ℭ𝔯𝔞𝔰𝔥𝔢𝔯 𝔓𝔬𝔭𝔨𝔦𝔡𝔛𝔱𝔢𝔠𝔥");
      ...
      await zk.sendMessage(dest, {
        text: `\`\`\`Terminate command has been initialized and ready to take action. POPKID will now kick ${usersToRemove.length} group members in a blink.\n\nGoodbye pals.\n\nThis process cannot be undone at this point!\`\`\``,
